...
export function Dashboard() {
  ...
  if (error) {
    return (
      <Layout title="Dashboard">
        <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 p-4 rounded-xl flex items-center space-x-3">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs transition-colors">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">Daily support volume vs detected phishing threats (14 Days)</p>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs transition-colors">
            ...
            <div className="flex justify-center space-x-4 text-xs font-medium text-slate-600 dark:text-slate-400">
            ...
            <div className="h-60">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={charts.categories} layout="vertical">
                  ...
                  <YAxis dataKey="category" type="category" tick={{ fontSize: 11, fill: tickColor }} width={120} />
                  <Tooltip contentStyle={{ backgroundColor: '#0F172A', color: '#FFF', borderRadius: '8px', border: '1px solid #334155', fontSize: '12px' }} />
            ...
            <div className="h-60">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={charts.threatTypes}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={gridColor} />
                  <XAxis dataKey="threat_type" tick={{ fontSize: 10, fill: tickColor }} interval={0} />
                  <YAxis tick={{ fontSize: 11, fill: tickColor }} />
                  <Tooltip contentStyle={{ backgroundColor: '#0F172A', color: '#FFF', borderRadius: '8px', border: '1px solid #334155', fontSize: '12px' }} />
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <button 
                ...
                className="text-xs text-blue-600 dark:text-blue-400 font-semibold hover:underline flex items-center space-x-1 cursor-pointer"
              >
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {recent.threats.length === 0 ? (
                <p className="p-4 text-xs text-slate-500 dark:text-slate-400 text-center">No security threats recorded.</p>
              ) : (
                recent.threats.map(threat => (
                  <div 
                    key={threat.threat_id} 
                    onClick={() => navigate(`/conversations/${threat.conversation_id}`)}
                    className="p-3.5 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors cursor-pointer flex items-center justify-between"
                  >
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs font-semibold text-slate-900 dark:text-white">{threat.customer_name}</span>
                        <RiskBadge level={threat.risk_level} />
                      </div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{threat.threat_type} — {threat.reason}</p>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-bold text-rose-600 dark:text-rose-400">{threat.risk_score} Score</span>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500">{new Date(threat.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {recent.unresolved.length === 0 ? (
                <p className="p-4 text-xs text-slate-500 dark:text-slate-400 text-center">No unresolved complaints pending.</p>
              ) : (
                recent.unresolved.map(item => (
                  ...
                  >
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs font-semibold text-slate-900 dark:text-white">{item.customer_name}</span>
                        <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700">{item.channel}</span>
                      </div>
                      <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5 font-medium">{item.issue}</p>
                    </div>
                    <div className="text-right">
                      <PriorityBadge level={item.priority} />
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{new Date(item.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))
              )}
