    ...
    <Layout title="Analyze Conversation">
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {PRESET_CASES.map((preset, idx) => (
              <button
                ...
                className="px-3 py-1.5 bg-white border border-blue-200 hover:border-blue-400 text-blue-800 text-xs font-semibold rounded-lg shadow-2xs hover:bg-blue-50 transition-colors cursor-pointer"
              >
                {preset.title}
              </button>
            ))}
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs">
          <h3 className="text-base font-bold text-slate-900 mb-4">Input Conversation Data</h3>
          ...
          <form onSubmit={handleAnalyze} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-hidden"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-hidden"
                >
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Customer Message *</label>
              <textarea
                rows={4}
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                placeholder="Paste customer email, support chat message, or phishing body here..."
                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-hidden font-mono"
              />
            </div>

            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Conversation History (Optional)</label>
              <textarea
                rows={2}
                value={history}
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider text-blue-700 border-b border-slate-100 pb-2">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    <span className="text-slate-500 font-medium">Issue Summary:</span>
                    <p className="font-semibold text-slate-900">{result.issue}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-900">{result.emotion}</p>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium">Urgency:</span>
                    <p className="font-semibold text-slate-900">{result.urgency}</p>
                  </div>
                  <div>
                ...
                <div className="pt-2 border-t border-slate-100 space-y-2 text-xs">
                  <div>
                    ...
                    <p className="text-slate-600 mt-0.5 leading-relaxed bg-slate-50 p-2.5 rounded-lg border border-slate-100">{result.summary}</p>
                  </div>
                  <div>
                    <span className="font-semibold text-slate-700">Customer Request:</span>
                    <p className="text-slate-600 mt-0.5 bg-slate-50 p-2.5 rounded-lg border border-slate-100">{result.customer_request}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider text-rose-600 border-b border-slate-100 pb-2">
                  Security Threat & Phishing Intelligence
                </h4>

                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-slate-500 font-medium">Threat Detected:</span>
                    <p className={`font-semibold ${result.security?.threat_detected ? 'text-rose-600' : 'text-emerald-600'}`}>
                      {result.security?.threat_detected ? 'YES' : 'NO'}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium">Threat Type:</span>
                    <p className="font-semibold text-slate-900">{result.security?.threat_type || 'None'}</p>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium">Social Engineering:</span>
                    <p className="font-semibold text-slate-900">{result.security?.social_engineering ? 'Detected' : 'None'}</p>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium">Credential Harvesting:</span>
                    <p className={`font-semibold ${result.security?.credential_request ? 'text-rose-600' : 'text-slate-700'}`}>
                      {result.security?.credential_request ? 'Yes (Password requested)' : 'No'}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium">OTP / 2FA Interception:</span>
                    <p className={`font-semibold ${result.security?.otp_request ? 'text-rose-600' : 'text-slate-700'}`}>
                      {result.security?.otp_request ? 'Yes (OTP code requested)' : 'No'}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium">Coercive Language:</span>
                    <p className="font-semibold text-slate-900">{result.security?.suspicious_message ? 'Urgency / Fear language' : 'Normal tone'}</p>
                  </div>
                </div>
                ...
                {result.security?.techniques && result.security.techniques.length > 0 && (
                  <div className="pt-2 border-t border-slate-100">
                    <span className="text-xs font-semibold text-slate-700">Detected Techniques:</span>
                ...
                {result.urls && result.urls.length > 0 && (
                  <div className="pt-2 border-t border-slate-100">
                    <span className="text-xs font-semibold text-slate-700">Extracted URLs ({result.urls.length}):</span>
                    <div className="mt-1 space-y-1">
                      {result.urls.map((u, i) => (
                        <div key={i} className="p-2 bg-slate-50 border border-slate-200 rounded text-[11px] space-y-1 font-mono">
                          <div className="flex items-center justify-between text-slate-800">
                            <span className="font-bold truncate max-w-xs">{u.url}</span>
                            <span className={`px-1.5 py-0.5 rounded text-[10px] ${u.risk_score >= 25 ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>
                              Score: {u.risk_score}
                            </span>
                          </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Recommended Agent / SOC Action</h4>
              <p className="text-sm font-semibold text-slate-900 bg-blue-50/70 p-3 rounded-lg border border-blue-100 text-blue-900">
