...
import { isSupabaseConfigured, testSupabaseConnection, syncAllToSupabase } from './db/supabase.js';
...
// Initialize SQLite database and auto-seed if empty
initDB();
try {
  const rowCount = db.prepare('SELECT COUNT(*) as count FROM conversations').get()?.count || 0;
  if (rowCount === 0) {
    console.log('🌱 Database empty on startup. Auto-seeding 60 realistic demo records...');
    seedDatabase(60);
  }
  syncAllToSupabase(db);
} catch (err) {
  console.warn('⚠️ Boot sync notice:', err.message);
}
